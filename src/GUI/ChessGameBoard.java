...
public class ChessGameBoard extends JPanel{
    ...
    private void createBoardSquare(int i, int j) {
    ChessGamePiece pieceToAdd = null;
    Color backgroundColor = (i + j) % 2 == 0 ? Color.WHITE : Color.BLACK;
    int colNum = i == 0 ? ChessGamePiece.BLACK : ChessGamePiece.WHITE;
    if (i == 1 || i == 6) { // black pawns or white pawns
    ...
    chessCells[i][j].setBackground(backgroundColor);
    chessCells[i][j].addMouseListener(listener);
    this.add(chessCells[i][j]);
}
    // ----------------------------------------------------------
